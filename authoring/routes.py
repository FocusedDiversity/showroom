...
def author_form():
    """Authoring entry form with palette and layout selectors."""
    models = list_models()
    db = get_db()
    ...
    return render_template('admin/author.html',
                           ...
                           palettes=palettes, layouts=layouts)
...
def author_generate():
    """Parse content, generate a single deck with selected palette + layout."""
    db = get_db()
    storage = get_storage()
    ...
    session_row = db.execute(
        """INSERT INTO authoring_sessions
           (title, markdown_content, model_name, theme_name, palette_slug, layout_slug, collage_id, status)
           VALUES (%s, %s, %s, %s, %s, %s, %s, 'previewing')
           RETURNING *""",
        (title, markdown_content, model_name, 'synaptiq',
         palette_slug, layout_slug,
         int(collage_id) if collage_id else None)
    ).fetchone()
    db.commit()
    session_id = session_row['id']

    # Generate single deck with selected palette + layout
    palette = get_palette(palette_slug, db)
    layout = get_layout(layout_slug, db)
    html = generate_deck(
        slides, palette, layout,
...
def preview_variants(session_id):
    ...
    palette_slug = session.get('palette_slug', 'arctic-breeze')
    layout_slug = session.get('layout_slug', 'editorial')
    palette = get_palette(palette_slug, db)
    layout = get_layout(layout_slug, db)
...
def apply_feedback(session_id):
    ...
    # Regenerate with palette + layout
    ...
    html = generate_deck(slides, palette, layout,
                         collage_data_uri=collage_data_uri,
...
def pptx_import():
    ...
    # Store images in session for the save step
    # (For simplicity, we skip persisting images in the preview step.
    #  They'll be re-extracted from the file if needed, or we can add
    #  temp storage later.)
...
def pptx_import_save():
    """Save the extracted palette and layout to the database."""
    from slugify import slugify
    db = get_db()
    ...
    if not palette_name or not layout_name:
        flash('Please provide names for both the palette and layout.', 'error')
        return redirect(url_for('authoring.pptx_import'))
    ...
    flash(f'Palette "{palette_name}" and layout "{layout_name}" saved!', 'success')
    return redirect(url_for('authoring.author_form',
                            palette=palette_slug, layout=layout_slug))


# ── Palette & Layout Preview API ─────────────────────────────────────
...
def layouts_list():
    """JSON list of available layouts."""
    db = get_db()
    layouts = list_layouts(db)
    return jsonify([{
        'slug': l.slug,
        ...
        'font_title': l.font_title,
        'font_content': l.font_content,
    } for l in layouts])
